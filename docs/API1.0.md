...
```js
GET  /api/mgr/ipinfo  HTTP/1.1
```
### 请求参数

http 请求消息 `url` 中 需要携带如下参数，`action`必填项，填写值为 `list_ipinfo`,`pagesize`必填项，分页的;每页获取多少条记录;`pagenum`必填项，获取第几页的信息;`keywords`可选项， 里面包含的多个过滤关键字，关键字之间用 `空格` 分开
...
```js
DELETE  /api/mgr/ipinfo  HTTP/1.1
Content-Type:   application/json
```
### 请求参数
